...
def ez_derive_text():
    ...
    try:
        ...
        #The options dictionary that is to be provided as part of the request payload. 
        #Please check the API guide for more parameters
        #The * here denotes 'all the columns', so in this case we are 
        #deriving sentiments, glove, topic extraction for all the columns
        options = {"text_types":{"*":["sentiments","glove","topic extraction"]},
                   "return_columns":"yes"}

        #Calling the eazyml library function for deriving numeric predictors
        response = eazyml.ez_derive_text(auth_token, model_id, options)
        print("Output of ez_derive_text function is", response)
    except Exception as e:
